...
import { AddProjectslist } from "../ProjectslistDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("projects", node, req, newRecordId, authData);

     const insertResult = await AddProjectslist(newRecordId, mutatedObj, req, authData);
