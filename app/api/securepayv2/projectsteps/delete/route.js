...
import { DeleteProjectstepslist } from '../projectstepslist/ProjectstepslistDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_project_steps_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'project_steps'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteProjectstepslist(deleteTokenDecode, whereStr);
