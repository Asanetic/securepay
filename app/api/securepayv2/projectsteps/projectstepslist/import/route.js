...
import { AddProjectstepslist } from "../ProjectstepslistDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("project_steps", node, req, newRecordId, authData);

     const insertResult = await AddProjectstepslist(newRecordId, mutatedObj, req, authData);
