...
import { DeleteDocumentslist } from '../documentslist/DocumentslistDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_documents_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'documents'; // Replace with your actual table
    
     
          
          const deleteAttachedMedia = await mosyQddata('documents', 'primkey', deleteTokenDecode);
          
          const fileToDelete = deleteAttachedMedia?.file_url
         
          mosyDeleteFile(fileToDelete);
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteDocumentslist(deleteTokenDecode, whereStr);
