...
import { AddDocumentslist } from "../DocumentslistDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("documents", node, req, newRecordId, authData);

     const insertResult = await AddDocumentslist(newRecordId, mutatedObj, req, authData);
